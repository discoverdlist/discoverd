  ...
  useEffect(() => {
    ...
    const fetchBot = async () => {
      try {
        ...
        if (data.owner) {
          try {
            const ownerRes = await fetch(`/api/users/${data.owner}`);
            if (ownerRes.ok) {
  ...
  const shareBot = () => {
    if (navigator.share) {
      navigator
        .share({
          ...
          text:
            botData?.description ||
            (lang === "en"
              ? "Check out this Discord bot!"
              : "Confira este bot para Discord!"),
      ...
      navigator.clipboard
        ...
        .catch((err) =>
          console.error(
            lang === "en" ? "Error copying: " : "Erro ao copiar: ",
            err
          )
        );
    }
        ...
        {router.query.edit === "success" && (
          ...
          >
            <div className="flex items-center">
              ...
              <div>
                <p className="font-bold">{t("botPage.editSuccess")}</p>
            ...
            <div className="flex flex-col md:flex-row items-center md:items-start gap-6">
              ...
              <div className="flex-1 text-center md:text-left">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
                  <div className="flex items-center justify-center md:justify-start bg-gray-50 dark:bg-gray-900/50 p-2 rounded-lg">
                    ...
                    <span className="text-gray-700 dark:text-gray-300">
                      {Number(botData.votes).toLocaleString(
                        lang === "en" ? "en-US" : "pt-BR"
                      )}{" "}
                      {t("botCard.votes")}
                    </span>
                  ...
                  <div className="flex items-center justify-center md:justify-start bg-gray-50 dark:bg-gray-900/50 p-2 rounded-lg">
                    ...
                    <span className="text-gray-700 dark:text-gray-300">
                      {Number(botData.servers).toLocaleString(
                        lang === "en" ? "en-US" : "pt-BR"
                      )}{" "}
                      {t("botCard.servers")}
                    </span>
                  ...
                  <div className="flex items-center justify-center md:justify-start bg-gray-50 dark:bg-gray-900/50 p-2 rounded-lg">
                    ...
                    <span className="text-gray-700 dark:text-gray-300">
                      {new Date(botData.createdAt).toLocaleDateString(
                        lang === "en" ? "en-US" : "pt-BR"
                      )}
                    </span>
                  </div>
                ...
                <div className="flex flex-wrap justify-center md:justify-start gap-3">
                  <Link
                    href={`/${lang}/bots/${botData._id}/vote`}
                    className="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-lg text-white bg-primary hover:bg-primary/90 transition-colors shadow-sm hover:shadow"
                  >
                  ...
                  >
                    ...
                    {t("botPage.addToServer")}
                  </Link>
                  {/* New button to view owner profile */}
                  {botData.owner && (
                    <Link
                      href={`/${lang}/users/${botData.owner}`}
                      className="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-lg text-white bg-blue-600 hover:bg-blue-700 transition-colors shadow-sm hover:shadow"
                    >
            ...
            <div className="bg-gray-50 dark:bg-gray-900/50 p-4 border-t border-gray-100 dark:border-gray-700/50 flex flex-wrap gap-4 justify-center md:justify-start">
              ...
              {botData.support && (
                ...
                >
                  ...
                  {lang === "en" ? "Support Server" : "Servidor de Suporte"}
                </a>
              )}
            </div>
          )}
